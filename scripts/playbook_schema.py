...
# Configure logging
logger = logging.getLogger('playbook schema')
...
class PlaybookSchema:
    """
    A class to transform vendor-specific playbooks
    """
    ...
    def run(self):
        """
        Run transformation for either a file or directory
        """
        ...
        if os.path.isfile(self.path):
            self._transform_file(self.path, results)
        ...
        # If the path is a directory, transform all files in it
            ...
            for root, dirs, files in os.walk(self.path):
                for file in files:
                    ...
                    self._transform_file(file_path, results)
        ...
        with open(f"../schemas/{self.platform}.json", "w", encoding="utf8") as f:
            json.dump(results, f)

    def _transform_file(self, filename, my_dict):
        """
        Trigger platform-specific transformation for a file

        :param filename: str, the name of the file to transform
        :param my_dict: dict, the dictionary to update with the transformed data
        ...
        try:
            ...
            if handler is not None:
                my_dict = handler.map_values(my_dict)
...
if __name__ == "playbook_schema._transform_file":
    # print error message if this module is imported
    logger.error(
        "This converter is currently meant to be used as a CLI tool only.")
    logger.error(
        "Use 'python3 -m playbook_schema._tranform_file -h' in your console for additional help.")

if __name__ == '__main__':
    # create an argument parser and add arguments
    parser = argparse.ArgumentParser(
        description="A playbook to JSON-schema exporter.")
    # required arguments
    parser.add_argument(
    ...
    try:
        # initialize the playbook2Schema object and transform the playbooks
        logger.debug('Start transformation of playbooks')
        transformator = PlaybookSchema(args.platform, args.path)
        transformator.run()
